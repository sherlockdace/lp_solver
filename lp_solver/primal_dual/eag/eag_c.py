...
@chex.dataclass
class EAGVState(StandardLPState):
    vec_y: jnp.ndarray
...
def eag_c_solve(
    ...
    The extragradient accelerated gradient algorithm is a primal-dual method for solving this problem.
    ...
    Returns:
        `EAGVState`: The state of the extragradient algorithm.
    """
...
def _cond_fn(eg_state: EAGVState) -> bool:
...
def _update_fn(eg_state: EAGVState) -> EAGVState:
    """Update the state of the extragradient algorithm."""
    alpha = eg_state.alpha
    curr_iter = eg_state.curr_iter

    halpern_x = (eg_state.vec_x_init - eg_state.vec_x) / (curr_iter + 2)
    halpern_y = (eg_state.vec_y_init - eg_state.vec_y) / (curr_iter + 2)
    vec_x_new = (
        eg_state.vec_x
        + halpern_x
        - alpha * (eg_state.vec_c - jnp.dot(eg_state.mat_A.T, eg_state.vec_y))
    )
    vec_x_new = jnp.clip(vec_x_new, 0, None)
    vec_y_new = (
        eg_state.vec_y
        + halpern_y
        + alpha * (eg_state.vec_b - jnp.dot(eg_state.mat_A, eg_state.vec_x))
    )
    vec_x_new = (
        eg_state.vec_x
        + halpern_x
        - alpha * (eg_state.vec_c - jnp.dot(eg_state.mat_A.T, vec_y_new))
    )
    vec_x_new = jnp.clip(vec_x_new, 0, None)
    vec_y_new = (
        eg_state.vec_y
        + halpern_y
        + alpha * (eg_state.vec_b - jnp.dot(eg_state.mat_A, vec_x_new))
    )
    ...
    new_state = EAGVState(
        ...
        vec_x=vec_x_new,
        vec_y=vec_y_new,
        vec_x_init=eg_state.vec_x_init,
        vec_y_init=eg_state.vec_y_init,
        alpha=alpha,
